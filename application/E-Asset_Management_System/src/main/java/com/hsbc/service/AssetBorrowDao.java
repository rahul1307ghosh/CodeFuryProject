...
public class AssetBorrowDao {
	...
	public static void borrowAsset(int assetId, int userId) {
		int period = 0;
		String query = "SELECT assetCategory.lending_period FROM asset, assetCategory WHERE asset.type = assetCategory.category AND asset.assetId="+assetId+";";
		...
		try {
			...
			String insertQuery = "Insert into assetAllocation(assetId,empId,allocation_date,due_date) values("+assetId+", "+userId+" ," +"CURDATE(), DATE_ADD(CURDATE(), INTERVAL "+period+" DAY));";
